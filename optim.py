...
class ADOPT(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.99), eps=1e-5,
...
def _single_tensor_adopt(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        denom = exp_avg_sq.sqrt().maximum(eps * torch.ones_like(exp_avg_sq))
        if step_t == 1:
            exp_avg.add_(grad.div(denom))
        elif step_t > 1:
...
def _multi_tensor_adopt(params: List[Tensor],
    ...
    for ((
        ...
        device_eps = [eps * torch.ones_like(x) for x in device_params]
        # denom = torch._foreach_maximum(device_exp_avg_sqs, device_eps)
        # torch._foreach_sqrt_(denom)
        denom = torch._foreach_maximum(torch._foreach_sqrt(device_exp_avg_sqs), device_eps)

        if device_state_steps[0] == 1:
            torch._foreach_add_(device_exp_avgs, torch._foreach_div(device_grads, denom))
        elif device_state_steps[0] > 1:
